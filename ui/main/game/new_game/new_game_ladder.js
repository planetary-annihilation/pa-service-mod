...
$(document).ready(function () {
    function SlotViewModel(options /* ai economy_factor */) {
        ...
        self.containsThisPlayer = ko.computed(function () {
            return self.playerId() === model.ladderPlayerId();
        });
    ...
    function NewGameViewModel() {
       ...
       // Click handler for leave button
        ...
        /* unused legacy list; localStorage 'systems' now holds the misc-db pointer and must not be rewritten from here */
        ...
        {
            self.transitPrimaryMessage(loc('!LOC:Requeuing'));
            self.transitSecondaryMessage('');
            self.transitDestination('coui://ui/main/game/start/start.html?startMatchMaking=true');
            self.transitDelay(0);
            window.location.href = 'coui://ui/main/game/transit/transit.html';
            return;
        };
    ...
    handlers.player_id = function(message)
    {
        model.ladderPlayerId(message.payload);
    }
