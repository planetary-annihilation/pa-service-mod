...
require(["api"], function (api) {
  ...
  $(document).ready(function () {
    ...
    // Settings overlay (opens in-place over the editor, same as the in-game ESC menu).
    model.showSettings = ko.observable(false);
    model.showSettings.subscribe(function (visible) {
      try { engine.call('game.allowKeyboard', !visible); } catch (e) {}
      // The uberbar (social tabs) and chat are separate panels layered above the editor,
      // so they'd cover the settings overlay. Hide them while it's open, restore on close.
      try { api.Panel.message('uberbar', 'visible', { value: !visible }); } catch (e) {}
      try { api.Panel.message('chat', 'visible', { value: !visible }); } catch (e) {}
      var p = api.panels && api.panels['settings'];
      if (p) {
        if (visible) p.focus();
        _.delay(p.update);
      }
    ...
    handlers = {};
    // settings.html (running as an embedded panel) sends this to its parent on Back.
    handlers['settings.exit'] = function () { model.showSettings(false); };
