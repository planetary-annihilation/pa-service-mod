...
$(document).ready(function () {
    ...
    loadSceneMods('load_planet');

    app.registerWithCoherent(model, handlers);

    ko.applyBindings(model);

    // Auto-load a specific system when arriving from the main menu's "Last Edited System".
    (function () {
        var autoLoadSystem = $.url().param('autoLoadSystem');
        if (!autoLoadSystem)
            return;
        $.when(model.userSystems.ready).always(function () {
            var systems = model.systems();
            for (var i = 0; i < systems.length; i++) {
                if (systems[i] && systems[i].name === autoLoadSystem) {
                    model.selectedSystemIndex(i);
                    model.loadSystem();
                    return;
                }
            }
